...
export const updateCart = async (req: Request, res: Response) => {
  try {
    ...
    if (itemType === 'Product') {
      ...
      itemPrice = item?.discountedPrice || item?.sellingPrice || item.price; // Use discountedPrice if available, else use sellingPrice
    ...
    if (existingItem) {
      ...
      existingItem.price = itemPrice * quantity;
    } else {
    ...
    res.status(200).json({ message: `${itemType} updated in cart`, cart });
  } catch (err) {
